...
beforeEach(() => {
  ...
  fs.mkdirSync(path.join(tmpDir, '.agents', 'skills', 'pc-plan-apply'), { recursive: true })
  fs.writeFileSync(
    path.join(tmpDir, '.agents', 'skills', 'pc-plan-apply', 'SKILL.md'),
...
describe('patchGuardrails optional tool sections', () => {
  it('injects selected guidance and clears unselected guidance', async () => {
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.agents', 'skills', 'pc-plan-apply', 'SKILL.md'), 'utf-8')
  ...
  it('migrates caveman guidance to the selected Simple English skill', async () => {
    const guardrailsDir = path.join(tmpDir, '.agents', 'skills', 'pc-guardrails-generic')
    ...
    expect(content).toContain('PC-GUARDRAILS-SIMPLE-ENGLISH-START')
