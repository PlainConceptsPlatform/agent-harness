...
<Callout title="Keep a shell task as the final gate" type="warn">
  ...
  Inside `/plan-goal`, `/plan-apply` step 10 runs lint, typecheck, test, and build commands and is
  the checks gate; `/repo-verify` then writes `verification-plan.md` — a reproduction journey for a
  later agent-browser executor. The shell task independently proves the critical commands passed
